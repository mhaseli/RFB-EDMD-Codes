%% RFB-EDMD Algorithm Application for Yeast Glycolysis System
% This script implements the Recursive Forward-Backward Extended Dynamic Mode 
% Decomposition (RFB-EDMD) algorithm on the Yeast Glycolysis system. The algorithm 
% is applied multiple times with varying epsilon values to analyze sensitivity 
% to this parameter.
...
% Load pre-processed data from saved_data directory
load('saved_data/trajectory_data.mat');

% Define parameters for dictionary generation
numStateVars = 7;  % Number of state variables in the system
maxPolyDegree = 4; % Maximum degree of polynomial terms in dictionary.
...
DY = dictionary(Y);  % Transform subsequent states
...
save(fullfile(output_dir, 'RFB_EDMD_data.mat'), ...
     'rfb_C_cell', 'rfb_subspace_dim', 'epsilon_values', 'dictionary');