...
% This script visualizes the relative prediction error of the RFB-EDMD model
% across the state space. It computes and plots the normalized dictionary
% prediction error for different epsilon values.
...
xMin = 0; xMax = 2;
yMin = 0; yMax = 2;
stepSize = 0.025; % Resolution of the grid
...
for i = 1:size(gridPoints, 1)
    ...
    y0 = gridPoints(i, :);
    % Store the final state in the finalPoints matrix
    finalPoints(i, :) = dynamicMap(y0);
end
...
for ii = 1:length(epsilon_values)
    ...
    hColorbar = colorbar;
    colormap('jet')
    clim([0 25]); % Limit color scale to 0-25%
    ...
    saveas(figDictionaryError, fullfile(figuresDir, ['Error_discreteSystem_FBEDMD_eps_1e-4_times_' num2str(epsilon_values(ii)*10000) '.png']));
    print(figDictionaryError, fullfile(figuresEpsDir, ['Error_discreteSystem_FBEDMD_eps_1e-4_times_' num2str(epsilon_values(ii)*10000)]), '-depsc');
end
