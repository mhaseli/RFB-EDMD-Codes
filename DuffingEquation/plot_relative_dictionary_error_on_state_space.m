...
for ii = 1:length(epsilon_values)
    ...
    hColorbar = colorbar;
    colormap('jet')
    clim([0 10]); % Limit color scale to 0-10%
    ...
    saveas(figDictionaryError, fullfile(figuresDir, ['Error_Duffing_FBEDMD_epsilon_1e-4_times_' num2str(epsilon_values(ii)*10000) '.png']));
    print(figDictionaryError, fullfile(figuresEpsDir, ['Error_Duffing_FBEDMD_epsilon_1e-4_times_' num2str(epsilon_values(ii)*10000)]), '-depsc');
end
