...
for ii = 1:numEigenfunctions
    ...
    if ~isreal(eigvals(ii))  % Check if eigenvalue is complex
        ...
        saveas(figAbs, fullfile(figuresDir, ['AbsValue_Eigenfunc_Duffing_FBEDMD_' num2str(ii) '.png']));
        print(figAbs, fullfile(figuresEpsDir, ['AbsValue_Eigenfunc_Duffing_FBEDMD_' num2str(ii)]), '-depsc');
        ...
        colormap jet;
        % Save phase plots
        saveas(figPhase, fullfile(figuresDir, ['Phase_Eigenfunc_Duffing_FBEDMD_' num2str(ii) '.png']));
        print(figPhase, fullfile(figuresEpsDir, ['Phase_Eigenfunc_Duffing_FBEDMD_' num2str(ii)]), '-depsc');
        ...
        saveas(figReal, fullfile(figuresDir, ['Real_Eigenfunc_Duffing_FBEDMD_' num2str(ii) '.png']));
        print(figReal, fullfile(figuresEpsDir, ['Real_Eigenfunc_Duffing_FBEDMD_' num2str(ii)]), '-depsc');
    end
end
...
saveas(figEigenvalues, fullfile(figuresDir, 'Eigenvalues_UnitCircle_Duffing_FBEDMD.png'));
print(figEigenvalues, fullfile(figuresEpsDir, 'Eigenvalues_UnitCircle_Duffing_FBEDMD'), '-depsc');
...
for i = 1:size(gridPoints, 1)
    y0 = gridPoints(i, :);
    [t, ysim] = ode45(duffingEq, [0 sampleTime], y0);
...
for ii = 1:numEigenfunctions
    ...
    % Save error plots
    saveas(figError, fullfile(figuresDir, ['Error_Eigenfunc_Duffing_FBEDMD_' num2str(ii) '.png']));
    print(figError, fullfile(figuresEpsDir, ['Error_Eigenfunc_Duffing_FBEDMD_' num2str(ii)]), '-depsc');
end
